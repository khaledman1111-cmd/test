...
def scanner_loop():
    while True:
        now_utc = time.gmtime()
        cur_hour = now_utc.tm_hour
        # ابدأ التداول فقط بين 17:00 (شامل) و22:00 (غير شاملة) UTC
        if 17 <= cur_hour < 22:
            print(f"⏰ {cur_hour}:00 UTC - بدء سكان وتنفيذ التداول...")
            try:
            ...
            print(f"⏳ خارج أوقات التداول. السكربت سيتوقف لحين دخول الفترة بين 17:00 و22:00 UTC. (الآن: {cur_hour}:00 UTC)")
            # احسب كم بقي للساعة 17:00 القادمة
            now_min, now_sec = now_utc.tm_min, now_utc.tm_sec
            if cur_hour >= 22:
                # انتظر حتى الغد الساعة 17:00
                wait_sec = ((24 - cur_hour + 17) * 3600) - (now_min * 60 + now_sec)
            else:
                # انتظر حتى 17:00 اليوم
                wait_sec = ((17 - cur_hour) * 3600) - (now_min * 60 + now_sec)
            if 0 < wait_sec < SCAN_PAUSE_SEC:
                time.sleep(wait_sec)
            else:
                time.sleep(180)  # انتظر 3 دقائق ثم افحص الوقت من جديد
